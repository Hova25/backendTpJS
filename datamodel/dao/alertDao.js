...
module.exports = class AlertDao extends BaseDAO {
    ...
    getByTextAndCheckedFalseAndUserAccountId(text, useraccount_id, checked) {
        let checkedParams = ""
        if(checked!==undefined && checked===false){
            checkedParams = " AND checked = false "
        }
        return new Promise((resolve, reject) =>
            this.db.query(`SELECT * FROM ${this.tablename} WHERE text LIKE $1 AND useraccount_id=$2 ${checkedParams}`, [ text, useraccount_id ])
